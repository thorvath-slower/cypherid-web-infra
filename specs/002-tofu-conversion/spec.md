# Improvement Specification: Terraform → OpenTofu Conversion

**Branch**: `improvement-#002-tofu-conversion`  ·  **Spec dir**: `specs/002-tofu-conversion/`

**Created**: 2026-06-10 · **Status**: Draft · **Repo**: `cypherid-web-infra`

**Input**: Move `cypherid-web-infra` off Terraform and onto OpenTofu, cut the Terraform Cloud/Enterprise coupling, point state at the shared foundation backend with locking on, and tighten the deploy IAM — all without changing what the infrastructure *does*.

## Why

We're standardizing the whole CZ ID stack on **OpenTofu** (Constitution Principle II — no BUSL/SSPL in the shipped product). `cypherid-web-infra` is a `fogg`-generated Terraform repo that today pins an exact Terraform version, runs through Terraform Cloud workspaces, and stores state in per-account S3 buckets with locking disabled in two environments. None of that survives the portability bar, so this slice does the engine swap and closes the state-locking gap (`bug-#006`) and the over-broad deploy role (`bug-#007`) on the way through.
...
### User Story 1 - The repo runs on OpenTofu, not Terraform (Priority: P1)

As a platform engineer, every component and module in `cypherid-web-infra` initializes and validates under OpenTofu, so we own the engine and ship nothing BUSL-encumbered.

**Why this priority**: Everything else in the conversion depends on the engine actually accepting the configuration.

**Independent Test**: `tofu init -backend=false && tofu validate` succeeds in a representative set of components (e.g. `envs/dev/auth0`) and our own modules, with no `required_version` rejection.

**Acceptance Scenarios**:
1. **Given** a converted component, **When** `tofu validate` runs, **Then** it passes with no version-constraint error and no deprecation warnings we introduced.
...
**Independent Test**: No `tfe` provider, no `terraform/tfe` component, and no `TFC_*` runtime variables remain; `tofu validate` still passes.
...
- **FR-001**: Every `required_version` constraint MUST admit the pinned OpenTofu version; no exact Terraform pins (`=1.14.8`, `=1.3.6`) or floors above the running tool (`>= 1.14.8`, `~> 0.12.24`).
- **FR-002**: `fogg` MUST be removed entirely — no `fogg.yml`, no `fogg.tf`, no fogg headers, no fogg build scripts or CI. The previously-generated files become hand-maintained native OpenTofu (the `terraform.tf` in each stack is the source of truth). The repo MUST run on OpenTofu alone, with no generator and no Terraform Cloud/Enterprise.
...
- **FR-005**: All CLI tooling (Makefiles, `scripts/*.mk`, CI) MUST invoke `tofu`, not `terraform`.
...
- **SC-001**: `tofu validate` is clean on all of our own modules and on representative components (those whose modules resolve without private/SSH fetch).
...
- **SC-005**: Zero fogg residue — `git grep -i fogg` returns only intentional migration notes (this spec + the README history line); no `fogg.yml`/`fogg.tf`/fogg headers/fogg scripts/fogg CI remain, and the repo builds and validates with `tofu` alone.
...
- The `fogg_ci.yml` workflow, replaced by `tofu_ci.yml` (native `tofu fmt -check` + `validate` on changed stacks; no auto-apply).
