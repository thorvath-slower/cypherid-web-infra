...
- **FR-002**: `fogg.yml` (the source of truth) MUST carry the OpenTofu version and provider set, so a later `fogg apply` reproduces the hand-applied generated-file edits. *(fogg is not available in this environment; the generated files are edited directly and `fogg.yml` updated to match — see Notes.)*
